 ...
 * 
 *      This firmware is written for two PIC 18F44K22 micro controllers.
 *      One PIC is the MASTER and the other is the SLAVE.
 *      A #define statement at the top of the Project.h file 
 *      directs the compiler to configure the microcontroller 
 *      to implement I2C master or slave mode. The master sends
 *      a test message and the slave responds with a second message.
 *
 ...
 * 
 *      8-14-22: All issues fixed so I2C communication between master and slave works flawlessly.
 */
#include "Project.h"
#include "Config.h"
    ...
    */
    ...
    {        
        ...
        if (I2CDataBytesReceived)
        {
            ...
            sprintf(I2C_READdata, "#%d: ABCDEFRGHI", trial++);
        ...
        for (i = 0; i < MAXBUFFER; i++) InBytes[i] = OutBytes[i] = 0;
        errorFlag = i2c_SlaveRead(SLAVE_ADDRESS, length, InBytes);
